<template>
  <div v-if="isOpen" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
    <div class="bg-white rounded-lg w-full max-w-md p-6 relative">
      ...
      <Form @submit="onSubmit" :validation-schema="schema" :initial-values="initialValues" v-slot="{ errors, meta }">
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-textdark">Name</label>
            <Field name="name" type="text" class="w-full border-grayout border p-2 rounded focus:border-primary" />
            <span class="text-red-600 text-sm">{{ errors.name }}</span>
          </div>
          <div>
            <label class="block text-sm font-medium text-textdark">Email</label>
            <Field name="email" type="email" class="w-full border-grayout border p-2 rounded focus:border-primary" />
            <span class="text-red-600 text-sm">{{ errors.email }}</span>
          </div>
          <div>
            <label class="block text-sm font-medium text-textdark">Position</label>
            <Field name="position" type="text" class="w-full border-grayout border p-2 rounded focus:border-primary" />
            <span class="text-red-600 text-sm">{{ errors.position }}</span>
          </div>
          <div>
            <label class="block text-sm font-medium text-textdark">Phone</label>
            <Field name="phone" type="text" class="w-full border-grayout border p-2 rounded focus:border-primary" />
            <span class="text-red-600 text-sm">{{ errors.phone }}</span>
          </div>
          <div>
            <label class="block text-sm font-medium text-textdark">Department</label>
          ...
          <div>
            <label class="block text-sm font-medium text-textdark">Salary</label>
            <Field name="salary" type="text"
              class="w-full border-grayout border p-2 rounded focus:border-primary" />
            <span class="text-red-600 text-sm">{{ errors.salary }}</span>
          </div>
        </div>

        <div class="mt-6 flex justify-end space-x-3">
          <Button @click="close" class="px-4 py-2 bg-red-500 text-gray-800 rounded hover:bg-red-500 transition">
            Cancel
          </Button>
