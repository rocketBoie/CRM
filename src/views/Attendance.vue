  ...
  <div class="flex min-h-screen">
    ...
    <div class="flex-grow p-8">
      <main>
        ...
        <div
          v-else
          class="overflow-x-auto shadow-md border border-gray-200 bg-white"
        >
          <table class="min-w-full w-full divide-y divide-gray-200">
            ...
            <tbody class="divide-y divide-gray-100">
              <tr
                ...
                :key="emp.id"
                @click="goToEmployeeCalendar(emp)"
                class="cursor-pointer hover:bg-gray-100"
              >
                <template v-if="emp.attendanceData">
                  ...
                  <td class="px-6 py-4 text-sm text-gray-700">
                    {{ calculateMonthlyStats(emp.attendanceData).totalPresent }}
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-700">
                    {{ calculateMonthlyStats(emp.attendanceData).halfDayLeave }}
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-700">
                    {{ calculateMonthlyStats(emp.attendanceData).fullDayLeave }}
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-700">
                    {{ calculateMonthlyStats(emp.attendanceData).paidLeave }}
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-700">
                    {{ getDaysInMonth(selectedYear, selectedMonth).length }}
                  </td>
                  <td class="px-6 py-4 text-sm text-gray-700">
                    {{
                      getAttendancePercentage(
                        calculateMonthlyStats(emp.attendanceData)
                      )
                    }}
                  </td>
                </template>
